...
이 MFCC feature 값과 tfidf score feature 값을 model train_input으로 사용하여(label = sentence index) 모델을 만들고 사람의 음성, 예를들어 "I love you so much"를 input으로 이 모델에 넣어주면 텍스트의 유사도 뿐만 아니라 오디오 특징값을 함께 계산하여 DB에 저장되어있는 가장 비슷한 sentence를 찾아줄 수 있을 것입니다.
